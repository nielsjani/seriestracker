...
const browserSyncServer = browserSync.create("seriestracker");
...
gulp.task("build:app:src", ["build:app:src:js", "build:app:src:index", "build:app:src:index:deploy", "build:app:src:css", "build:app:src:img"]);
gulp.task("watch:app:src", ["watch:app:src:js", "watch:app:src:index", "watch:app:src:index:deploy", "watch:app:src:css", "watch:app:src:img"]);
...
gulp.task("serve", callback => {
    ...
    let frontendProxy = httpProxy(
        "/seriestracker",
        {
            ...
            pathRewrite: {"^/seriestracker": ""}
        });
    browserSyncServer.init(
        {
            server: {
                baseDir: paths.distSrc,
                middleware: [backendProxy, frontendProxy]
            },
            port: 8080,
            ui: {port: 8081},
            files: `${paths.distSrc}**/*`
